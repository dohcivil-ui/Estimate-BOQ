...
- **audit (2 มิ.ย. 69):** ตำรา วทค.ชลบุรี บท 4.7 ยืนยัน ลวดผูก = 30 กก./ตัน = 0.03 → ตรงกับค่าที่ใช้ใน P3-1 (cross-check ผ่าน, secondary source)
...
### B6. waste% เหล็กเส้นรายขนาด ❌ gate
- **ปัญหา:** code ใช้ waste เหล็กเส้น flat 7% ทุกขนาด → undercount เหล็กเส้นใหญ่
- **ตำรา (วทค.ชลบุรี ตาราง 4.1/4.2 — secondary):** waste รายขนาด 5–15% (RB6→5% .. RB25→15%; DB10→7% .. DB22→15%)
- **สถานะ:** รอหลักเกณฑ์ฉบับหลัก กบก. ยืนยันก่อนแก้ — ห้ามแตะ math

---

## หนี้เชิงพฤติกรรม / coverage gap (ไม่ใช่ค่าคงที่ — เป็น behavior/architecture)

### C1. slab either/or — AI draft หายเมื่อมี marks
- **ที่:** buildBOQ.ts:318
- **อาการ:** พอเติม markDims แม้ตัวเดียว ระบบสลับไปใช้ทาง marks ทั้งก้อน → slab ที่มาจาก AI แต่ยังไม่เติม markDims หายหมด
- **แนวแก้ (อนาคต):** คง provisional slab จาก AI ที่ยังไม่ถูก override แทน either/or

### C2. พื้นยัง derive จาก W×L ไม่ได้
- **อาการ:** areaSqm กรอกมือช่องเดียว ไม่มี geometric derive จากกว้าง×ยาว
- **แนวแก้ (อนาคต):** เพิ่ม W×L → areaSqm พร้อม validator เทียบ tag sum (validator >5% มีแล้วจาก P3-3)
